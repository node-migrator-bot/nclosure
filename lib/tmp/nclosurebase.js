...
var PATH = require('path'),
    IO = require('./io'),
    FS = require('fs'),
    SETTINGSLOADER = require('settingsloader.js');
 ...
 * @type {nclosure.opts}
 */
nclosure.base.globalClosureSettings;
 ...
 * @type {Object.<boolean>}
 */
nclosure.base.scriptsWritten_ = {};

/**
 * @private
 * @type {nclosure.settingsLoader}
 */
nclosure.base.settingsLoader_ = SETTINGSLOADER.settingsLoader;



/**
 * If this method returns false then the loadScript function above will abort
...
//    var contents = FS.readFileSync(process.argv[1]).toString();
...
nclosure.base.prototype.resolveClosureBasePath_ = function () {
    return PATH.resolve(
        PATH.join(this.globalClosureSettings.closureBasePath, 'closure', 'goog'));
};
...
nclosure.base.prototype.loadScript = function (dir, file) {
    // TODO: this.closureScriptLoading always returns true
    // Give goog.js a chance to stop the loading of certain scripts
    if (!this.closureScriptLoading(dir, file)) {
        return;
    }

    // TODO: Do we need absolute file support?
    var path = (IO.isRoot(file) ? file : PATH.resolve(dir, file));

    if (this.scriptsWritten_[path]) {
        return;
    }
...
nclosure.base.prototype.loadDependenciesFile = function (dir, file) {
    var path = PATH.normalize(PATH.join(dir, file));
...
nclosure.base.prototype.loadCurrentScriptDeps_ = function () {
    ...
    this.loadDependenciesFile(__dirname, PATH.join('..', 'bin', 'deps.js'));
