 ...
 * @deprecated Use PATH.resolve(PATH.normalize(PATH.join(basedir, file))); instead
 */
nclosure.settingsLoader.prototype.getPath = function(baseDir, file) {
  if (baseDir && baseDir.charAt(baseDir.length - 1) !== IO.PATH_SEPARATOR) baseDir += IO.PATH_SEPARATOR;
  if (file.charAt(0) === IO.PATH_SEPARATOR) file = file.substring(1);
  var path = baseDir + file;
  var cwd = process.cwd() + IO.PATH_SEPARATOR;
 ...
 * @deprecated Use PATH.dirname() instead
 */
nclosure.settingsLoader.prototype.getFileDirectory = function(file) {
  if (PATH.existsSync(file) && FS.statSync(file).isDirectory()) {
      return file;
  }

  var didx = file.lastIndexOf(IO.PATH_SEPARATOR);
  if (didx < 0) {
      return '.' + IO.PATH_SEPARATOR;
  }
  return file.substring(0, file.lastIndexOf(IO.PATH_SEPARATOR) + 1);
};
...
nclosure.settingsLoader.prototype.readArgsFromJSONFile = function(path) {
  ...
  //var dir = path.substring(0, path.lastIndexOf(IO.PATH_SEPARATOR));
  return this.getOptsObject_(PATH.dirname(path), json);
};
...
nclosure.settingsLoader.prototype.readSettingsObject = function(opts) {
  ...
  var globalSettings =
      this.readArgsFromJSONFile(PATH.normalize(PATH.join(__dirname , '..', 'bin' , 'closure.json')));
  var currentDirSettings =
      this.readArgsFromJSONFile(PATH.normalize(PATH.join(process.cwd(), 'closure.json')));
...
nclosure.settingsLoader.prototype.parseCompilerArgsFromFileImpl_ =
  ...
  //var dirIdx = file.lastIndexOf(IO.PATH_SEPARATOR);
  //var dir = dirIdx < 0 ? '.' : file.substring(0, dirIdx);
    var dir = PATH.dirname(file);
    //this.getPath(process.cwd(), dir)
//        console.log('--------------------------------------------------------');
//        console.log(this.getPath(process.cwd(), dir));
//        console.log(PATH.resolve(dir));
//        console.log('--------------------------------------------------------');
  return this.validateOpsObject_(PATH.resolve(dir), opts || {}, true);
};
...
nclosure.settingsLoader.prototype.parseClosureBasePath_ = function(dir) {
  ...
  var path = require('path').normalize(pathToClosure.join(IO.PATH_SEPARATOR));
  return path;
};
 ...
 * @deprecated Use PATH.existsSync(dir) instead
 */
