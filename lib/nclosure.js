...
nclosure.nclosure = function (opts) {
    ...
    global.goog.writeScriptTag_ = function (filePath) {
        try {
            ...
            // dirty hack
            fileloader.load(PATH.normalize(PATH.join(PATH.dirname(APP_OPTIONS.additionalDeps[0]) , filePath)));
        }
        return false;
    };
    ...
    for(i = 0; i < depsLength; i += 1) {
        ...
        if (!NCUTIL.isPathRoot(APP_OPTIONS.additionalDeps[i])) {
            APP_OPTIONS.additionalDeps[i] = PATH.join(process.cwd(), APP_OPTIONS.additionalDeps[i]);
        }
        // load
