...
function loadOptions(opts) {
    ncoptions.mergeOptions(RM.load({
        path: DEFAULT_CLOSURE_CONF_PATH,
        type: RM.TYPES.CONF
    }));

    try {
        ncoptions.mergeOptions(RM.load({
            path: CURRENT_CLOSURE_CONF_PATH,
            type: RM.TYPES.CONF
        }));
    } catch (e) {
...
function loadGoog() {
    ...
    global.goog.writeScriptTag_ = function (filePath) {
        ...
        var path,
            paths = Array.prototype.concat.call(
                        ncoptions.get('closureBasePath'),
                        ncoptions.get('additionalDeps').every(function (item) { return PATH.dirname(item); })),
